...
function loadAssets() {
  try {
    ...
    if (!saved || typeof saved !== 'object' || Array.isArray(saved)) return { ...DEFAULT_ASSETS }
    return { ...DEFAULT_ASSETS, ...saved }
...
function seededCandles(symbol, tf, count = 72) {
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    p = close
    return { open, high, low, close, volume: 18000 + Math.abs(Math.sin(i * 0.8)) * 32000 + i * 120 }
  })
}
...
function App(){
  ...
  const [tf,setTf]=useState('5分足'),[tab,setTab]=useState('chart')
  const [settings,setSettings]=useState(false),[addOpen,setAddOpen]=useState(false)
  const [newCode,setNewCode]=useState(''),[newName,setNewName]=useState(''),[newSubtitle,setNewSubtitle]=useState('')
  ...
  const current=assets[symbol]||assets.NIKKEI225
  const activeSymbol=assets[symbol]?symbol:'NIKKEI225'
  const data=useMemo(()=>seededCandles(activeSymbol,tf),[activeSymbol,tf])
  ...
  const score=(last.close>s?1:-1)+(last.close>e?1:-1)+(ml>ms?1:-1)+(r<70?1:-1)
  const direction=score>=3?'上向き':score<=-3?'下向き':'中立'
  const max=Math.max(...data.map(x=>x.high)),min=Math.min(...data.map(x=>x.low))
  const setIndicator=(k,v)=>setIndicators(x=>({...x,[k]:v}))
  const addAsset=()=>{const code=newCode.trim().toUpperCase(),name=newName.trim();if(!code||!name||assets[code])return;setAssets(x=>({...x,[code]:{name,subtitle:newSubtitle.trim()||name,price:1000,change:0,type:'custom'}}));setSymbol(code);setNewCode('');setNewName('');setNewSubtitle('');setAddOpen(false)}
  const removeCustom=code=>{if(!assets[code]||assets[code].type!=='custom')return;setAssets(x=>{const y={...x};delete y[code];return y});if(activeSymbol===code)setSymbol('NIKKEI225')}
  ...
  return <div className="app">
    ...
    <main>
      <section className="ticker-tabs">{Object.entries(assets).map(([code,a])=><button key={code} className={activeSymbol===code?'ticker active':'ticker'} onClick={()=>setSymbol(code)}><span>{code==='NIKKEI225'?'指数':code}</span><b>{a.name}</b><strong>{a.price.toLocaleString()}円</strong><em className={a.change>=0?'up':'down'}>{a.change>=0?'▲':'▼'} {Math.abs(a.change)}円</em></button>)}<button className="ticker" onClick={()=>setAddOpen(true)}><Plus size={18}/><b>銘柄を追加</b><small>コード・名称を入力</small></button></section>
      <section className="price-card"><div><span className="eyebrow">{current.subtitle}</span><div className="price">{Math.round(last.close).toLocaleString()}<small>円</small></div><div className={pct>=0?'change up':'change down'}>{pct>=0?'▲':'▼'} {Math.abs(pct).toFixed(2)}% <span>前足比</span></div></div><div className={'signal '+direction}><span>テクニカル状況</span><b>{direction}</b><small>参考情報・投資判断ではありません</small></div></section>
      {tab==='chart'&&<><div className="toolbar"><div className="segmented">{TF.map(x=><button key={x} className={tf===x?'selected':''} onClick={()=>setTf(x)}>{x}</button>)}</div><button className="refresh" onClick={()=>setRefresh(!refresh)}><RefreshCw size={17}/> {refresh?'表示設定 ON':'表示設定 OFF'}</button></div>
      ...
      <section className="metrics">{indicators.rsi&&<Metric title="RSI (14)" value={r.toFixed(1)} hint={r>70?'過熱圏':r<30?'売られ過ぎ圏':'中立圏'}/>} {indicators.macd&&<Metric title="MACD" value={ml.toFixed(1)} hint={ml>ms?'シグナル上':'シグナル下'}/>}<Metric title="出来高" value={(last.volume/1000).toFixed(1)+'K'} hint={activeSymbol==='NIKKEI225'?'参考値':'デモ値'}/></section>
      <section className="detail-card"><div className="detail-title"><b>テクニカル状況</b><span>表示設定: {refresh?'ON':'OFF'}</span></div><div className="rows"><Row label="SMA20" value={Math.round(s).toLocaleString()+'円'} positive={last.close>s}/><Row label="EMA20" value={Math.round(e).toLocaleString()+'円'} positive={last.close>e}/><Row label="ボリンジャー" value={bb[bb.length-1]?`${Math.round(bb[bb.length-1].lower)} ～ ${Math.round(bb[bb.length-1].upper)}円`:'—'}/><Row label="総合状況" value={direction}/></div></section></>}
      {tab==='signals'&&<section className="signal-page"><div className="signal-hero"><span>現在のテクニカル状況</span><b>{direction}</b><small>複数のテクニカル指標から機械的に算出した参考情報</small></div><div className="signal-grid"><Row label="SMA20" value={last.close>s?'上回る':'下回る'} positive={last.close>s}/><Row label="EMA20" value={last.close>e?'上回る':'下回る'} positive={last.close>e}/><Row label="RSI14" value={r.toFixed(1)}/><Row label="MACD" value={ml>ms?'上向き':'下向き'} positive={ml>ms}/></div><p className="signal-warning">これは売買の推奨ではありません。市場データは現在デモデータです。</p></section>}
    ...
    {addOpen&&<div className="modal-backdrop" onClick={()=>setAddOpen(false)}><div className="modal" onClick={e=>e.stopPropagation()}><div className="modal-head"><h2>銘柄を追加</h2><button className="icon-btn" onClick={()=>setAddOpen(false)}><X/></button></div><label className="setting-row" style={{display:'block'}}><span>銘柄コード</span><input value={newCode} onChange={e=>setNewCode(e.target.value)} placeholder="例：7203" /></label><label className="setting-row" style={{display:'block'}}><span>銘柄名</span><input value={newName} onChange={e=>setNewName(e.target.value)} placeholder="例：トヨタ自動車" /></label><label className="setting-row" style={{display:'block'}}><span>説明（任意）</span><input value={newSubtitle} onChange={e=>setNewSubtitle(e.target.value)} placeholder="例：東証プライム" /></label><button className="primary-btn" onClick={addAsset}>追加する</button></div></div>}
    {settings&&<div className="modal-backdrop" onClick={()=>setSettings(false)}><div className="modal" onClick={e=>e.stopPropagation()}><div className="modal-head"><h2>表示設定</h2><button className="icon-btn" onClick={()=>setSettings(false)}><X/></button></div><SettingToggle label="SMA20" checked={indicators.sma} setChecked={v=>setIndicator('sma',v)}/><SettingToggle label="EMA20" checked={indicators.ema} setChecked={v=>setIndicator('ema',v)}/><SettingToggle label="RSI14" checked={indicators.rsi} setChecked={v=>setIndicator('rsi',v)}/><SettingToggle label="MACD" checked={indicators.macd} setChecked={v=>setIndicator('macd',v)}/><SettingToggle label="ボリンジャーバンド" checked={indicators.bb} setChecked={v=>setIndicator('bb',v)}/><SettingToggle label="通知表示" checked={notifications} setChecked={setNotifications} icon={notifications?<Bell size={18}/>:<BellOff size={18}/>}/><div className="custom-list"><h3>追加した銘柄</h3>{Object.entries(assets).filter(([,a])=>a.type==='custom').map(([code,a])=><div className="custom-item" key={code}><span><b>{code}</b> {a.name}</span><button onClick={()=>removeCustom(code)}>削除</button></div>)}{!Object.values(assets).some(a=>a.type==='custom')&&<small>まだありません</small>}</div></div></div>}
  </div>
}
