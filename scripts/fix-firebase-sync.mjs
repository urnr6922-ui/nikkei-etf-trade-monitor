...
const syncEffects = `  useEffect(()=>saveAssets(assets),[assets])\n  useEffect(()=>{if(!auth){setSyncReady(true);setSyncStatus('未接続');return}let alive=true;try{const unsubscribe=onAuthStateChanged(auth,async u=>{if(!alive)return;setUser(u);setSyncReady(false);if(!u){setSyncStatus('未接続');setSyncReady(true);return}try{if(!db){setSyncStatus('未接続');setSyncReady(true);return}const snap=await getDoc(doc(db,'users',u.uid));if(snap.exists()){const d=snap.data();if(d.assets&&typeof d.assets==='object')setAssets(x=>({...x,...d.assets}));if(d.refresh!==undefined)setRefresh(Boolean(d.refresh));if(d.notifications!==undefined)setNotifications(Boolean(d.notifications));if(d.indicators&&typeof d.indicators==='object')setIndicators(x=>({...x,...d.indicators}));setSyncStatus('同期済み')}else setSyncStatus('クラウド保存を開始')}catch{setSyncStatus('同期エラー')}finally{if(alive)setSyncReady(true)}});return()=>{alive=false;unsubscribe?.()}}catch{setSyncStatus('未接続');setSyncReady(true);return()=>{alive=false}}},[])\n  useEffect(()=>{if(!user||!syncReady||!db)return;const timer=setTimeout(()=>setDoc(doc(db,'users',user.uid),{assets,refresh,notifications,indicators,updatedAt:new Date().toISOString()},{merge:true}).then(()=>setSyncStatus('同期済み')).catch(()=>setSyncStatus('同期エラー')),350);return()=>clearTimeout(timer)},[user,syncReady,assets,refresh,notifications,indicators])\n  const signIn=()=>{if(!auth||!googleProvider){setSyncStatus('未接続');return Promise.resolve()};return signInWithPopup(auth,googleProvider).catch(()=>setSyncStatus('ログイン失敗'))}\n  const signOutUser=()=>{if(!auth)return Promise.resolve();return signOut(auth).catch(()=>setSyncStatus('ログアウト失敗'))}`
...
console.log('Firebase cloud sync patched safely')
