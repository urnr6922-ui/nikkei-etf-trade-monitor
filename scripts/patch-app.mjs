...
if (!s.includes('loadMarket(activeSymbol, tf===\'日足\')')) {
  ...
  s = s.replace(activeNeedle, activeNeedle + effect)
}
...
s = s.replace(' · {tf} · デモデータ', " · {tf} · {isLive?'実データ':'データ取得待ち'}{isLive&&liveRows?.fetchedAt?` · 取得 ${new Date(liveRows.fetchedAt).toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'})}`:''}")
